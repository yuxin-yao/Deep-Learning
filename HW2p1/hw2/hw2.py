...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        output_width = (input_width-kernel_sizes[0])//strides[0] + 1
        for i in range(1, len(num_channels), 1):
            output_width = (output_width-kernel_sizes[i])//strides[i] + 1
        self.linear_layer = Linear(output_width*num_channels[-1], num_linear_neurons,linear_weight_init_fn, bias_init_fn)
    ...
    def forward(self, x):
        ...
        for i in range(self.nlayers):
            ...
            self.output = self.activations[i].forward(self.convolutional_layers[i].forward(self.output))
        
        self.output = self.flatten.forward(self.output)
        self.output = self.linear_layer.forward(self.output)
    ...
    def backward(self, labels):
        ...
        for i in range(self.nlayers-1,-1,-1):
            ...
            grad = self.convolutional_layers[i].backward(self.activations[i].derivative()*grad)
