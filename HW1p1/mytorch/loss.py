...
class SoftmaxCrossEntropy(Criterion):
    ...
    def forward(self, x, y):
        ...
        LL = -np.sum(y*x,axis=1)
        a = np.mean(np.mean(x))
        RR = a + np.log(np.sum(np.exp(x-a),axis = 1))
        self.loss = LL + RR
        return self.loss
    ...
    def derivative(self):
        ...
        b = np.tile((np.sum(np.exp(self.logits),axis = 1)),(len(self.logits[0]),1)).T
        DD = -self.labels + np.exp(self.logits)/b
        return DD
